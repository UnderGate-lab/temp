...
from collections import OrderedDict, defaultdict  # [診断追加] defaultdict追加
import heapq
...
class HighlyOptimizedFrameRestorer:
    ...
    def __init__(self, device, video_file, preserve_relative_scale, max_clip_length,
                 ...
                 enable_diagnostics=True):  # [診断追加] 新パラメータ
    ...
    def stop(self):
        ...
        if hasattr(self, 'diagnostics') and self.diagnostics is not None:
            ...
            print("\n⚠️ 診断機能が無効です", file=sys.stdout)
            print(f"   enable_diagnostics = {getattr(self, 'enable_diagnostics', 'not set')}", file=sys.stdout)
    ...
    def _clip_restoration_worker_vram(self):
        ...
        while self.clip_restoration_threads_should_be_running:
            ...
            try:
                ...
                if clip is None:
                    logger.debug(f"{worker_name}: 停止 (処理数: {clips_processed})")
                    break

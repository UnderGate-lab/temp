...
class HighlyOptimizedFrameRestorer:
    ...
    def seek(self, target_ns):
            """
            [完全修正版] シーク - 全スレッドを確実に停止してから再起動
            """
            logger.info(f"🔄 シーク開始: target={target_ns}ns")
            ...
            # 2. mosaic_detector を先に停止(これがクリップ供給元)
            if hasattr(self, 'mosaic_detector'):
                self.mosaic_detector.stop()
                logger.debug("mosaic_detector 停止完了")
            
            # 3. 全キューにダミーデータを送信してスレッドをアンブロック
            for _ in range(self.parallel_clips + 5):
                try:
                    self.mosaic_clip_queue.put_nowait(None)
                except queue.Full:
            ...
            for thread in all_threads:
                if thread and thread.is_alive():
                    thread.join(timeout=2.0)
                    if thread.is_alive():
                        logger.warning(f"⚠️ スレッド {thread.name} が停止しませんでした")
            ...
            logger.info(f"✅ シーク準備完了 - 再起動開始")
            
            # 10. 新しい位置から再起動
            self.start(start_ns=target_ns)
            
            logger.info(f"✅ シーク完了: {target_ns}ns から再生中")
